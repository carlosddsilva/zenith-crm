...
export function DealForm({
    ...
    (async () => {
      try {
        const [cRes, pRes] = await Promise.all([
          fetch('/api/zenith/contacts'),
          fetch('/api/account/members'),
        ]);
        if (cancelled) return;

        const contactsData = cRes.ok ? await cRes.json() : [];
        const profilesData = pRes.ok ? await pRes.json() : [];

        setContacts(contactsData);
        setProfiles(profilesData);
      } catch (err) {
        console.error('Failed to load contacts/profiles', err);
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        // Assume API returns sorted list, grab newest
        setLinkedConversation(data?.[0] || null);
      } catch (err) {
  ...
  async function handleSave() {
    ...
    try {
      if (deal) {
        const res = await fetch(`/api/zenith/deals/${deal.id}`, {
          method: 'PATCH',
        ...
        const res = await fetch('/api/zenith/deals', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
        });
