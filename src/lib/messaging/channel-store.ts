...
export async function getDefaultServiceChannel(
  accountId: string,
): Promise<
  ResolvedMessagingChannel | null
> {
  const [channel] =
    await db
      .select()
      ...
      .where(
        and(
          ...
          eq(
            messagingChannels.isDefaultService,
            true,
          ),
        ),
      )
  ...
  if (
    ...
    if (
      ...
      throw new Error(
        "Invalid Meta messaging channel configuration",
      );
    }
  ...
  if (
    ...
    throw new Error(
      "Invalid Evolution messaging channel configuration",
    );
  }
