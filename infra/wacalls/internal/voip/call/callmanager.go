...
func (m *CallManager) RejectCall(ctx context.Context, callID string, reason core.EndCallReason) error {
	...
	/*
	 * Reject e signaling fire-and-forget.
	 *
	 * Diferente do offer, nao esperamos uma resposta
	 * via Query. O stanza precisa ser enviado antes
	 * de marcarmos a chamada como rejeitada localmente.
	 *
	 * O contexto continua valido enquanto o handler
	 * HTTP aguarda esta chamada retornar.
	 */
	...
	current := m.currentCall

	if current != nil &&
		current.CallID == callID &&
		!current.IsEnded() {

		if err := current.ApplyTransition(
			Transition{
				Type:   TransitionLocalRejected,
				Reason: reason,
			},
		); err != nil {
			m.mu.Unlock()
			...
			m.log.Error(
				"reject state transition failed",
				"call_id", callID,
				"err", err,
			)
...
func (m *CallManager) EndCall(ctx context.Context, reason core.EndCallReason) error {
	...
	/*
	 * O terminate precisa ser enviado antes
	 * de marcarmos a chamada como encerrada
	 * localmente.
	 */
	if _, err := m.sock.Query(ctx, node); err != nil {
		m.log.Error(
			"terminate send error",
			"call_id", callID,
			"err", err,
		)
	...
	if err := current.ApplyTransition(
		...
		m.log.Error(
			"terminate state transition failed",
			"call_id", callID,
			"err", err,
		)
