...
func (s *server) doWebRTC(sess *Session, w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.log.Error(
			"browser webrtc bridge failed",
			"call_id", callID,
			"offer_bytes", len(body.SDPOffer),
			"error", err,
		)
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": err.Error()})
		return
	...
	bridge.OnTerminalICE = func() {
		if !sess.reg.detachBridge(callID, bridge) {
			return
		}

		s.log.Info(
			"browser webrtc bridge detached",
			"call_id", callID,
		)
...
func (s *server) doReject(sess *Session, w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		writeJSON(
			w,
			http.StatusNotFound,
			map[string]string{"error": "no such call"},
		)
		return
	}

	if err := ac.cm.RejectCall(
		r.Context(),
		id,
		core.EndCallReasonDeclined,
	); err != nil {
		s.log.Error(
			"reject call failed",
			"session", sess.id,
	...
	/*
	 * Somente removemos a chamada local depois
	 * que o provider confirmou o RejectCall.
	 */
	sess.removeCall(id)

	s.broker.endCall(
		id,
		string(core.EndCallReasonDeclined),
	)
...
func (s *server) doEndCall(sess *Session, w http.ResponseWriter, r *http.Request) {
	...
	if err := ac.cm.EndCall(
		r.Context(),
		core.EndCallReasonUserEnded,
	); err != nil {
		s.log.Error(
			"end call failed",
			"session", sess.id,
	...
	s.broker.endCall(
		id,
		string(core.EndCallReasonUserEnded),
	)
